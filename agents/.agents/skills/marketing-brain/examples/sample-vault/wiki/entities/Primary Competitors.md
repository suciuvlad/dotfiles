...
Identified via the DataForSEO Labs `competitors_domain` endpoint (saved to `.raw/sources/dataforseo/site-competitors-domain-2026-05-11.json`) plus per-keyword SERP top-10 results for the priority keyword set. Numerical claims (positions, intersection counts, etv) trace to the digest in `.raw/sources/dataforseo/`.

## Tier 1 — Direct Head-to-Head Competitors

The competitors that show up repeatedly in the SERPs Demo Growth Co's site targets, on the same niche, technique, and location intents.
...
- Tier 1 set is grounded in DataForSEO data and per-keyword SERP top-10 evidence; revisit when GSC reveals which queries actually drive Demo Growth Co's residual traffic.
