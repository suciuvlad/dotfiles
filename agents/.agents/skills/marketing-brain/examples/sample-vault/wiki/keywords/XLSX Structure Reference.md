...
The marketing-brain skill produces a deduplicated, scored XLSX (`keywords-YYYY-MM-DD.xlsx`) as part of the 6-step research pipeline (Step 3). This note documents the per-sheet schema and how to read it. The companion in-vault view is [[keywords.base|keywords.base]] — same data, queryable inside Obsidian; the XLSX is for Excel power-users and for handing data to Demo Growth Co in a familiar format.
...
| Column | Type | Description |
|---|---|---|
| source_competitor | string | Which competitor's `ranked_keywords` pull surfaced this keyword (or "site" if from https://www.example.com's own pull) |
...
2. Pick the candidates that align with Demo Growth Co's capabilities.
...
The XLSX is regenerated whenever the skill re-runs Step 3. Re-run quarterly OR after a major Google update OR when Demo Growth Co adds a major new content cluster.
