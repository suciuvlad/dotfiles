...
- Pursuit pattern: identify pages on these properties that link out to relevant resources; ensure https://www.example.com has the canonical resource for what they would link to; reach out via the property's editorial contact when Demo Growth Co's resource is genuinely the best fit.
...
Every earned link or community citation is logged with date, source URL, anchor text (if a link), and the destination page on https://www.example.com. The aggregate count and per-class deltas roll up to `[[Dual Surface Scorecard]]` as a Leverage indicator. Monthly review compares delta to the acceptance threshold below.
...
- **Owner**: Demo Growth Co (or the named expert if different — assigned per the active overlay).
- **Verifier**: Strategy Owner (monthly review of earned-link / citation deltas).
...
- `[[E-E-A-T for saas]]` — why Authoritativeness signals matter post-HCU.
