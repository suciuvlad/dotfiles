...
- Demo Growth Co's confidence in factual accuracy (regulations, prices, dates, technical claims).

## Documentation Required

Per URL, save a `[[content-prune-decision]]` template instance to `wiki/decisions/prunes/` (subfolder created during execution of the prune phase). Each instance records the URL, the bucket assigned, the inputs above, the rationale, and the date executed. This creates an auditable history if Demo Growth Co later questions a removal or a redirect.
