...
- If Demo Growth Co observes a temporary yield dip after density reduction, that is expected and acceptable — recovery is the bigger prize.
- Re-evaluate monetization density after the next core update reassesses the site.

## Owner and Verifier

- Owner: Demo Growth Co (monetization platform dashboard, placement decisions).
- Verifier: Strategy Owner (PSI weekly check during sprint, sanity check on monetization platform recommendations).
- Escalation: any persistent CWV regression after density reduction triggers a hosting / CDN / infrastructure review (see [[Open Questions for Demo Growth Co]]).
