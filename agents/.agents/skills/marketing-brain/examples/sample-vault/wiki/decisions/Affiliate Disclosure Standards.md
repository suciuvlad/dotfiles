...
Hidden monetization is one of the clearest signals quality raters and the HCU classifier are tuned to detect. This decision sets the disclosure bar for every page on `https://www.example.com` that contains an affiliate link.
...
- Visible disclosure signals editorial honesty and helps establish the trust signal Demo Growth Co is trying to build / recover.
...
- Owner: Demo Growth Co (executes on every affiliate page).
- Verifier: Strategy Owner (spot-checks during refresh phase).
