...
This roadmap assumes **Day 0 starts the day Demo Growth Co completes [[Day 0 Measurement Access Gate]]**. All downstream dates shift if Day 0 slips. Days are working-days indicative, not strict calendar days — if Demo Growth Co is part-time, the calendar window stretches accordingly.

Owner per slice is named below. Verifier is Strategy Owner for every slice unless otherwise specified. Acceptance criteria carry from each linked flow.
...
Owner: Demo Growth Co. Until this gate closes, every recommendation in this brain is advisory.
...
- Install `claude-seo`.
- Run `/seo-audit` against `https://www.example.com`.
...
Owner: Demo Growth Co (with Strategy Owner scaffolding). Parallel to GSC export — does not block the loss-map work.
...
Owner: Strategy Owner (analysis), Demo Growth Co (CMS fixes).
...
Owner: Demo Growth Co (CMS execution), Strategy Owner (verification).
...
Owner: Demo Growth Co (with named expert input on first-person passages and proof artifacts).
...
Owner: Demo Growth Co + named expert.
...
Owner: Demo Growth Co + Strategy Owner.
...
- HCU recovery typically realizes at the next Google core update window. Per [[Recovery Scope and Expectations]], do not promise a date — set Demo Growth Co's expectations correctly.
