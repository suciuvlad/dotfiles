...
- **Pathway D — Outbound click to third-party booker / scheduler / partner.** GA4 event `external_referral` with parameters `target_platform` and `source_url`. Whether this pathway is live depends on Demo Growth Co's actual booking infrastructure.
...
- **Owner:** Demo Growth Co — configures GA4, the form, the click tracking, and the consent layer.
- **Verifier:** Strategy Owner — confirms each event fires correctly in GA4 DebugView before any consolidation ships, and confirms the cluster pivot exploration is saved.
...
The following items in [[Open Questions for Demo Growth Co]] block full execution of this plan and must be resolved before Acceptance can pass:
...
These are flagged as blocking. Do not assume defaults; confirm with Demo Growth Co before configuring.
...
- [[Open Questions for Demo Growth Co]] — where the blocking dependencies live.
