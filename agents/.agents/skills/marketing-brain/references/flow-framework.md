...
- Private editorial notes are used only to identify candidate concepts, never
  as public prose.
- The page should make sense to a human reader and to an AI agent that only
  has this repository.

---

## How `marketing-brain` Uses FLOW

The `beast-planner.md` subagent loads this file as the strategic backbone for
composing the ULTIMATE BEAST plan. The four-stage Find → Leverage → Optimize →
Win structure is the spine of the plan's body sections (sections 2–5 of the
deliverable). The Measurement section seeds the **Dual Surface Scorecard**
deliverable in `wiki/deliverables/`.

When the subagent assembles the plan it must:

1. Name the **search surface** for every recommendation (organic result, AI
   Overview, AI search citation, local pack, community discussion).
2. Cite **observable evidence** — every numerical claim traces to a JSON file
   in `.raw/sources/dataforseo/` or is marked "TBD pending [source]".
3. Sequence work by which **FLOW stage is blocking progress** (rather than
   running all four in parallel).

This is non-negotiable. See `references/beast-plan-prompt.md` for the full
synthesis prompt.

## See Also

- Source repo: <https://github.com/AgriciDaniel/flow>
- Bibliography: <https://github.com/AgriciDaniel/flow/blob/main/docs/10-references/bibliography.md>
- AI Search Surface Map: <https://github.com/AgriciDaniel/flow/blob/main/docs/01-framework/ai-search-surface-map.md>

## Sources

- Search Engine Journal, *State of SEO 2026*, retrieved 2026-04-25.
